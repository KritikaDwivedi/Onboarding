...
public class NestVerifyTravelRequestOnHomePageSteps {
	...
	public static void getTravelRequest(String request) {
		ConfigurationManager.getBundle().setProperty("requestId",CommonStep.getText("viewTravelRequest.requestId.txt.loc"));
		ConfigurationManager.getBundle().setProperty("travelPurpose",CommonStep.getText("viewTravelRequest.travelPurpose.txt.loc"));
	}
	...
	public static void verifyActionConfirmation(){
		...
		Validator.verifyTrue(!request.equals((String)ConfigurationManager.getBundle().getProperty("requestId")), "Action is not performed successfully", "Action is performed successfully");
		}
	...
	public static void openTravelRequest() {
		QAFExtendedWebElement request= new QAFExtendedWebElement(String.format(ConfigurationManager.getBundle().getString("viewTravelRequest.request.info.loc"),(String)ConfigurationManager.getBundle().getProperty("travelPurpose"),(String)ConfigurationManager.getBundle().getProperty("requestId")));
		Validator.verifyTrue(request.isDisplayed(), "Request is not opened successfully", "Request is opened successfully");
		}
